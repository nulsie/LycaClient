...
async function openFollowersModal() {
    ...
    document.body.style.overflow = 'hidden'; 
    list.innerHTML = '<div style="text-align:center; padding: 20px; color: var(--text-muted);">Loading followers...</div>';
    
    try {
        const baseUrl = authSession ? AUTH_API_BASE : API_BASE;
        const headers = authSession ? { 'Authorization': `Bearer ${authSession.accessJwt}` } : {};
        
        // Fetch up to 50 followers
        const res = await fetch(`${baseUrl}/app.bsky.graph.getFollowers?actor=${currentHandle}&limit=50`, { headers });
        if (!res.ok) throw new Error("Failed to fetch followers");
        
        const data = await res.json();
        
        if (!data.followers || data.followers.length === 0) {
            list.innerHTML = '<div style="text-align:center; padding: 20px; color: var(--text-muted);">No followers yet.</div>';
            return;
        }
        ...
        data.followers.forEach(follower => {
            const avatar = follower.avatar || 'https://via.placeholder.com/40';
            const name = escapeHTML(follower.displayName || follower.handle);
            const handle = escapeHTML(follower.handle);
            ...
            item.innerHTML = `
                <img src="${avatar}" class="follower-avatar" alt="avatar">
                <div class="follower-info">
                    <span class="follower-name">${name}</span>
                    <span class="follower-handle">@${handle}</span>
                </div>
            `;
        ...
        list.innerHTML = `<div style="text-align:center; padding: 20px; color: #ff3b30;">Error: ${err.message}</div>`;
    }
}
